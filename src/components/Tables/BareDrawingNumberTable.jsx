...
import {
  getAllBareDrawingNumberData,
  getaSearch,
  GetSingleBareDrawingNumberData,
} from "../../services/BareDrawingNumberService";
...
const BareDrawingNumberTable = () => {
  ...
  useEffect(() => {
    dispatch(getAllBareDrawingNumberData(page));
  }, [page]);
        ...
        <table>
          ...
          {data[0].data.map((res) => (
            <React.Fragment key={res._id}>
              <tr>
                <td>{res.BareDrawingNumber}</td>
                <td>{res.OtoO}</td>
                <td>{res.Intermediate}</td>
                <td>{res.RO}</td>
                <td>{res.Bowl}</td>
                <td>{res.Angle}</td>
                <td>{res.VOff}</td>
          ...
          {modal ? (
            <BareDrawingNumberMIntermediateal
              id={ids}
              setModal={setModal}
